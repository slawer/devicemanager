...
namespace DeviceManager
{
    ...
    {
        private Application app = null;                     // констекст
...
"Во время вычисления коэффициентов афинного преобразования возникла ошибка. Строка с ошибкой удалена", 
                    ...
                    else
                    {
                        ...
                        MessageBox.Show(this,
"Во время вычисления коэффициентов афинного преобразования возникла ошибка. Таблица очищена",
                                        "Информация", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
        ...
        {
            get { return checkBoxSetNumberAuto.Checked; }
        }
        ...
        private void selectParameter_Click(object sender, EventArgs e)
        {
            ...
            if (first != null)
            {
                r_frm.Position = first.Index;
            }

            if (r_frm.ShowDialog(this) == DialogResult.OK)
            {
                textBoxSelectedParameter.Tag = r_frm.SelectedParameter;
                textBoxSelectedParameter.Text = r_frm.SelectedParameter.Macros.Description;

                first.Index = r_frm.SelectedParameter.Position;
                transformation.Args[0].Index = first.Index;
            }
        }
        ...
        {
            if (checkBoxSetNumberAuto.Visible && checkBoxSetNumberAuto.Checked == false)
            {
                foreach (Formula formula in app.Converter.Formuls)
                {
            ...
            if (first.Index == -1)
            {
                MessageBox.Show(this, "Определите калибруемый параметр", "Предупреждение", 
        ...
        {
            app.Converter.OnComplete -= Converter_OnComplete;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void checkBoxCalcMedia_CheckedChanged(object sender, EventArgs e)
        {
            if (!checkBoxCalcMedia.Checked)
            {
                media.Reset();
